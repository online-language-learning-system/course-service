...
public class ModuleServiceImpl extends ModuleServiceGrpc.ModuleServiceImplBase {
    ...
    public void getModuleIdsForCourse(CourseIdRequest request,
        ...
        try {
            List<Long> moduleIds = courseModuleService
                                        .getAllModuleByCourseId(request.getCourseId())
                                        .stream()
                                        .map(module -> module.id())
                                        .toList();

            ModuleListResponse moduleListResponse = ModuleListResponse.newBuilder()
                            .addAllModuleIds(moduleIds)
                            .build();

            responseObserver.onNext(moduleListResponse);
